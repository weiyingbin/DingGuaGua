...
public class ReadingFragment extends Fragment implements View.OnClickListener {
    ...
    private Handler toBookHandle = new Handler() {
        @Override
        public void handleMessage(Message msg) {
            super.handleMessage(msg);
            mBookPageFactory = new BookPageFactory(mContext, mBook, mShelfId);
    ...
    public static ReadingFragment newInstance(Integer shelfId) {
        Bundle args = new Bundle();
        args.putInt(SHELF_ID, shelfId);
    ...
    private void initData() {
        ...
        bookHandle = new BookHandle(mContext);
        mDefaultDialog = new ProgressDialog(mContext);
        mDefaultDialog.setMessage("正在加载...");
        mDefaultDialog.setCanceledOnTouchOutside(false);
        mShelfId = getArguments().getInt(SHELF_ID);//获取传过来的书架id
        Shelf shelf = shelfService.getShelf(mShelfId);
        if (shelf != null) {
            //shelfToBook(shelf);
            String text = null;
            try {
                text = bookHandle.localTxtToText(shelf.getLocalPath());
            } catch (Exception e) {
                e.printStackTrace();
            }
            mBook = new Book(shelf.getName(), null, text);
            mBookPageFactory = new BookPageFactory(mContext, mBook, mShelfId);
        } else {
            Toast.makeText(mContext, "找不到书架信息", Toast.LENGTH_SHORT).show();
        }
        mBgColors = new int[]{
                0xffe7dcbe,  //复古
                0xffffffff,  // 常规
    ...
    private void shelfToBook(final Shelf shelf) {
        mDefaultDialog.show();
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                toBookHandle.sendMessage(message);
            }
        });
    }
    ...
    private void initViews(View v) {
        ...
        mLabelPopup = new LabelPopup(mContext, mShelfId);
    }
    ...
    private class BatteryPowerReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            //首次获取电量后初始化flipView
            if (isFirstRead) {
                ReadInfo readInfo = SaveHelper.getObject(mContext, mShelfId + SaveHelper.DRAW_INFO);
                if (readInfo != null) {
                    mPageList = mBookPageFactory.drawCurTwoPages(mPowerPercent);
                    mFlipView.updateBitmapList(mPageList);
